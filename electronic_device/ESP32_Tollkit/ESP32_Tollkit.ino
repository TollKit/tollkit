...
#define SS_PIN 5
#define RST_PIN 2
const char* servidor_mqtt= "172.20.10.13"; //mosquitto
int puerto = 1883;
...
void loop() {
  ...
  if (estado_actual == 0){
    ...
    lcd.setCursor(0,1);
    lcd.print("vacuancion (QR)"); // Mensaje a desplegar
    delay(5000);
    estado_actual = 1;
    }
  if (estado_actual == 1){
  ...
  if(estado_actual==2){
    ...
    for (byte i = 0; i < mfrc522.uid.size; i++) { // bucle recorre de a un byte por vez el UID
          ...
          if(comparaUID(LecturaUID, Usuario1)){    // llama a funcion comparaUID con Usuario1
            client.loop();
            client.publish("rfid", "Jhomar Astuyauri"); //id
            delay(2000);
            Serial.println("Bienvenido Richard Elescano");
            estado_actual = 3;
            lcd.clear();
            lcd.setCursor(0,0) ;
            lcd.print("Alumno:"); // Mensaje 2 a desplegar
            lcd.setCursor(0,1) ;
            lcd.print("Jhomar Astuyauri"); // Mensaje 2 a desplegar
            delay(2000);
